import type { LinesMap, PathsMap, RouteResult, RouteStep } from "@workspace/metro-core/types";
...
export const exportRouteImage = async ({
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const height =
        headerHeight + filteredSteps.length * (stepHeight + stepGap) + footerHeight + padding * 2;
      canvas.width = width;
      ...
      const appTitle = isRTL ? "تهگو" : "TehGo";
      ...
      const fromStationX = isRTL
        ? infoX - ctx.measureText(fromLabel).width - 15
        : infoX + ctx.measureText(fromLabel).width + 15;
      ...
      const toStationX = isRTL
        ? infoX - ctx.measureText(toLabel).width - 15
        : infoX + ctx.measureText(toLabel).width + 15;
      ...
      filteredSteps.forEach((step, idx) => {
        ...
        const lineColor = lines[step.line]?.color || "#6b7280";
      ...
      stepPositions.forEach(({ y: stepY, lineColor, step, originalIdx }, idx) => {
        ...
        const guideText = isFirstStep
          ? getFirstStepGuide(route, lines, paths, lang, getStationDisplay)
          : isTransfer
            ? getTransferGuide(route, originalIdx, lines, paths, lang, getStationDisplay)
            : "";
        const hasGuide = guideText.length > 0;
        const badgeHeight = hasGuide ? 48 : 32;
        ctx.font = `bold 16px ${fontFamily}`;
        let lineBadgeWidth = ctx.measureText(lineName).width + 32;
        if (hasGuide) {
          ctx.font = `12px ${fontFamily}`;
          lineBadgeWidth = Math.max(lineBadgeWidth, ctx.measureText(guideText).width + 32);
        }
        ctx.font = `bold 16px ${fontFamily}`;
        ctx.fillStyle = lineColor;
        drawRoundedRect(
      ...
      ctx.fillStyle = textSecondary;
      ctx.textAlign = "center";
      const footerText = isRTL ? "ساخته شده با ❤️ برای مسافران مترو تهران" : "Made with ❤️ for Tehran Metro passengers";
      ...
      canvas.toBlob(async (blob) => {
        ...
        const filename = `tehgo-route-${Date.now()}.jpg`;
