...
// ─── DotQRCanvas ──────────────────────────────────────────────────────────────
// Standalone canvas component that renders the QR with circular/rounded dots.
// Does not depend on the QRCode context — use it as a drop-in alternative.
...
interface DotQRCanvasProps extends React.ComponentProps<"canvas"> {
  value: string
  size: number
  foreground?: string
  level?: QRCodeLevel
...
function DotQRCanvas({
  ...
  const canvasRef = React.useRef<HTMLCanvasElement>(null)
  const composedRef = useComposedRefs(ref, canvasRef)
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      ...
      const qr = QRLib.create(value, { errorCorrectionLevel: level })
      ...
      const dotFraction = 0.82

      for (let row = 0; row < count; row++) {
        for (let col = 0; col < count; col++) {
          if (!modules.get(row, col)) continue

          const cx = (col + margin) * cellSize
          const cy = (row + margin) * cellSize
          const d = cellSize * dotFraction
          const pad = (cellSize - d) / 2

          // finder pattern corners stay as rounded-square; data modules are circles
          const isFinder =
            (row < 7 && col < 7) ||
            (row < 7 && col >= count - 7) ||
            (row >= count - 7 && col < 7)
