...
export type FileUploadItem = {
  ...
  error?: string
  file?: File
  /** Remote URL for an already-uploaded image (shown as the thumbnail). */
  previewUrl?: string
}

/** Object URL (or remote previewUrl) for image rows, so we can show a thumbnail. */
function useRowPreview(item: FileUploadItem): string | null {
  ...
  useEffect(() => {
    ...
    setUrl(null)
  }, [item.file, item.previewUrl, item.type])
  return url
}
...
function FileUploadRow({
  ...
  const previewUrl = useRowPreview(item)
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={cn(
            "grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl bg-muted text-muted-foreground",
            classNames?.leading
          )}
        >
          {previewUrl ? (
        ...
        <div className={cn("min-w-0 flex-1", classNames?.content)}>
            ...
            <div className="flex shrink-0 items-center gap-1">
              ...
              {status === "error" ? (
                <button
                  ...
                  aria-label={`تلاش مجدد ${item.name}`}
                  className={cn(
          ...
          {showProgress ? (
            <div
              ...
              aria-label={`پیشرفت بارگذاری ${item.name}`}
              className={cn(
...
export function FileUpload({
  ...
  description = "فایل‌ها را به صف بارگذاری اضافه کنید",
      ...
      <button
        ...
        className={cn(
          ...
          centered
            ...
            : "items-center gap-4 p-5 text-right",
          classNames?.dropzone
        )}
      >
        <motion.span
        ...
        <span className={cn("min-w-0", centered ? "max-w-xs" : "flex-1")}>
          ...
          >
            {maxReached ? "به سقف بارگذاری رسیدید" : title}
          </span>
          <span
            className={cn(
          ...
          >
            {maxReached
              ? `${items.length} از ${maxFiles} فایل اضافه شد`
              : description}
          </span>
        </span>
