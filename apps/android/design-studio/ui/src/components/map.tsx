...
// Register the RTL text plugin once on the client. Without it, MapLibre renders
// Arabic/Persian labels disconnected and in reversed order (e.g. "س ل ا م"
// instead of "سلام"). Lazy-loaded so it only fetches when RTL text is present.
let rtlTextPluginLoaded = false
function ensureRTLTextPlugin() {
  if (rtlTextPluginLoaded || typeof window === "undefined") return
  rtlTextPluginLoaded = true
  // getRTLTextPluginStatus is "unavailable" until set; guard against StrictMode
  // double-invoke and any prior registration to avoid throwing.
  if (MapLibreGL.getRTLTextPluginStatus() !== "unavailable") return
  MapLibreGL.setRTLTextPlugin(
    "https://unpkg.com/@mapbox/mapbox-gl-rtl-text@0.3.0/dist/mapbox-gl-rtl-text.js",
    true // lazy: only download when a label with RTL text is encountered
  )
}
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="absolute end-0.5 top-0.5 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-sm text-foreground transition-colors hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    >
...
/** Build a closed geodesic ring (approx) around a center for a circle polygon. */
...
type MapCircleProps = {
  /** Optional unique identifier for the circle layers */
  id?: string
  ...
  fillOpacity?: number
  /** Outline color (default: fillColor) */
  lineColor?: string
  /** Outline width in px (default: 2) */
  lineWidth?: number
  /** Number of segments used to approximate the circle (default: 64) */
  steps?: number
}

/**
 * Renders a filled geodesic circle (e.g. a delivery zone) centered on a point.
 * Follows the same source/layer lifecycle as <MapRoute>.
 */
function MapCircle({
  id: propId,
  center,
  ...
  const stroke = lineColor ?? fillColor

  // GeoJSON sources have no built-in geometry transition (unlike paint
  // properties), so a radius change is tweened by hand: each frame rebuilds
  // the ring at an eased-interpolated radius and pushes it to the source.
  const animatedRadiusRef = useRef(radiusMeters)
  const prevCenterRef = useRef(center)
  const rafRef = useRef<number | null>(null)

  const setRingData = useCallback(
    (r: number) => {
      const source = map?.getSource(sourceId) as
        | MapLibreGL.GeoJSONSource
        | undefined
      source?.setData({
        ...
        geometry: {
          type: "Polygon",
          coordinates: [buildCircleRing(center, r, steps)],
        },
      })
    },
    [map, sourceId, center, steps]
  )
  ...
  useEffect(() => {
    ...
    animatedRadiusRef.current = radiusMeters
    ...
    map.addSource(sourceId, {
      ...
      data: {
        ...
        geometry: {
          type: "Polygon",
          coordinates: [buildCircleRing(center, radiusMeters, steps)],
        },
      },
    })
    ...
    map.addLayer({
      ...
      paint: { "line-color": stroke, "line-width": lineWidth },
    })
  ...
  // Ease toward the new radius when it changes. A center change (e.g. a
  // draggable marker) snaps instantly — only the radius grows/shrinks smoothly.
  useEffect(() => {
    if (!isLoaded || !map) return
    ...
    const startRadius = animatedRadiusRef.current
    const targetRadius = radiusMeters

    if (centerChanged || startRadius === targetRadius) {
      animatedRadiusRef.current = targetRadius
      setRingData(targetRadius)
      return
    }
    ...
    const tick = (now: number) => {
      ...
      const r = startRadius + (targetRadius - startRadius) * easeOutCubic(t)
      animatedRadiusRef.current = r
      setRingData(r)
  ...
  }, [isLoaded, map, center, radiusMeters, setRingData])

  // Update paint when colors change
  useEffect(() => {
    if (!isLoaded || !map || !map.getLayer(fillLayerId)) return
    map.setPaintProperty(fillLayerId, "fill-color", fillColor)
    map.setPaintProperty(fillLayerId, "fill-opacity", fillOpacity)
    if (map.getLayer(lineLayerId)) {
      map.setPaintProperty(lineLayerId, "line-color", stroke)
...
export type { MapRef, MapViewport, MapArcDatum, MapArcEvent, Theme }
