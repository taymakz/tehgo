...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  const revealClipPadding = useMemo(() => {
    // Edge headroom so a smooth curve's overshoot (curveNatural) and the stroke
    // width aren't clipped at the top/bottom of the plot area.
    const EDGE_HEADROOM = 24
    if (!composedBarDataKeys?.length) {
      return EDGE_HEADROOM
    }
    ...
    >
      <ChartProvider value={contextValue}>
        ...
        >
          ...
          >
            {/* Hit area spans the full width (incl. side margins) so hovering
                near the curve's first/last point still activates it — the
                interaction clamps out-of-range x to the edge data point. */}
            <rect
              fill="transparent"
              height={innerHeight}
              width={width}
