  ...
  const yScales = useMemo(() => {
    ...
    return buildYScalesForLines({
      ...
      resolveDomain: (dataKeys) => {
        // Stacked bars render the CUMULATIVE sum of every series at each data
        // point, so the axis ceiling must be based on that per-point sum —
        // not each series' own individual max (which is what a grouped chart
        // needs). Using the individual max here understates the domain
        // whenever more than one series has a value on the same point, so
        // the stack's top renders past the axis (and since the chart
        // container is `overflow-visible`, well past the chart itself).
        let max = 0
        for (const d of data) {
          if (stacked) {
            let sum = 0
