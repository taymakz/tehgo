...
// Vector-tile basemap labels (street/place names) are rendered by MapLibre's
// own WebGL glyph renderer, which has no Unicode text-shaping support out of
// the box — Arabic/Persian letters render in isolated form instead of
// joined. This plugin adds that shaping. It only affects native map labels;
// our own DOM-rendered markers/popups already shape correctly via the
// browser's normal text engine and don't need it.
if (typeof window !== "undefined" && MapLibreGL.getRTLTextPluginStatus() === "unavailable") {
  MapLibreGL.setRTLTextPlugin(
    "https://unpkg.com/@mapbox/mapbox-gl-rtl-text@0.3.0/dist/mapbox-gl-rtl-text.js",
    true,
  ).catch(() => {
    // Non-fatal: basemap labels fall back to unshaped glyphs if this fails to load.
  });
}
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col overflow-hidden rounded-md border border-zinc-800 bg-zinc-900 text-zinc-50 shadow-sm [&>button:not(:last-child)]:border-b [&>button:not(:last-child)]:border-zinc-800 dark:border-zinc-200 dark:bg-white dark:text-zinc-900 dark:[&>button:not(:last-child)]:border-zinc-200">
      {children}
    </div>
  );
}
