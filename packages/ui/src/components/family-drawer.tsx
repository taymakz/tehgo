...
interface FamilyDrawerRootProps {
  ...
  /** Controlled current view. Falls back to internal state when omitted. */
  view?: string
  defaultView?: string
...
function FamilyDrawerTrigger({
  ...
  return (
    <Drawer.Trigger asChild>
      <button
        className={clsx(
          "fixed top-1/2 left-1/2 antialiased -translate-y-1/2 -translate-x-1/2 h-[44px] rounded-full border bg-background px-4 py-2 font-medium text-foreground transition-colors hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring md:font-medium cursor-pointer",
          className
        )}
        type="button"
      >
...
function FamilyDrawerOverlay({ className, onClick }: FamilyDrawerOverlayProps) {
  ...
  return (
    <Drawer.Overlay
      className={clsx("fixed inset-0 z-40 bg-black/30", className)}
  ...
  if (asChild) {
    return (
      <Drawer.Content
        asChild
        className={clsx(
          "fixed inset-x-4 bottom-4 z-40 mx-auto max-w-[420px] overflow-hidden rounded-[36px] border border-border bg-popover text-popover-foreground outline-none md:mx-auto md:w-full",
          className
        )}
      >
        <Slot>{content}</Slot>
...
function FamilyDrawerClose({
  ...
  const defaultClose = (
    <button
      data-vaul-no-drag=""
      className={clsx(
        "absolute end-5 top-5 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-muted text-muted-foreground transition-transform focus:scale-95 focus-visible:ring-2 focus-visible:ring-ring active:scale-75 cursor-pointer",
        className
      )}
      type="button"
    >
...
function FamilyDrawerButton({
  ...
  const button = (
    <button
      data-vaul-no-drag=""
      className={clsx(
        "flex h-12 w-full items-center gap-[15px] rounded-[16px] bg-muted px-4 text-[17px] font-semibold text-foreground transition-transform focus:scale-95 focus-visible:ring-2 focus-visible:ring-ring active:scale-95 md:font-medium cursor-pointer",
        className
      )}
      onClick={onClick}
      type="button"
...
function FamilyDrawerSecondaryButton({
  ...
  const button = (
    <button
      ...
      type="button"
      className={clsx(
        "flex h-12 w-full items-center justify-center gap-[15px] rounded-full text-center text-[19px] font-semibold transition-transform focus:scale-95 focus-visible:ring-2 focus-visible:ring-ring active:scale-95 md:font-medium cursor-pointer",
        className
      )}
      onClick={onClick}
    >
